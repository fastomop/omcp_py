...
class SyntheaWorkflow:
    ...
    async def setup_environment(self) -> bool:
        ...
        try:
            ...
            for package in packages:
                resp = await self.client.call_tool("install_package", {
    ...
    async def create_omop_schema(self) -> bool:
        ...
        try:
            ...
            resp = await self.client.call_tool("create_omop_schema", {
                "sandbox_id": self.sandbox_id
            })
    ...
    async def load_synthea_data(self) -> bool:
        ...
        try:
            ...
            resp = await self.client.call_tool("load_synthea_to_postgres", {
                "sandbox_id": self.sandbox_id,
    ...
    async def run_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Basic counts
            resp = await self.client.call_tool("analyze_omop_data", {
                "sandbox_id": self.sandbox_id,
                "analysis_type": "basic"
            })
            ...
            for operation in llm_operations:
                resp = await self.client.call_tool("llm_dataframe_operation", {
                    "sandbox_id": self.sandbox_id,
                    "operation": operation
                })
                
                if resp.get("exit_code") == 0:
                    results["llm_operations"][operation] = json.loads(resp.get("output", "{}"))
    ...
    async def cleanup(self):
        ...
        if self.sandbox_id:
            try:
                await self.client.call_tool("remove_sandbox", {
                    "sandbox_id": self.sandbox_id,
                    "force": True
                })

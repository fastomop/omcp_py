...
# Optional: pass --load-duckdb to load synthetic_data/synthea.duckdb into Postgres using scripts/bootstrap_synthea.py
# Usage:
...
if [ "$LOAD_DUCKDB" = true ]; then
  ...
  # Ensure bootstrap uses the host-accessible DB (localhost)
